...
3. 🚨 줄바꿈 및 형태 (가장 중요) 🚨: 
   - 성우가 호흡하기 편하도록 **모든 문장(마침표, 물음표, 느낌표)이 끝날 때마다, 혹은 긴 문장의 쉼표마다 반드시 줄바꿈(Enter)을 하라.**
   - 대사를 하나의 거대한 텍스트 덩어리(문단)로 이어서 쓰는 것을 엄격히 금지한다. 한 줄에 2문장 이상을 배치하지 마라.
   - [잘못된 예시]: 낡은 담벼락, 빛바랜 간판, 그리고 사람들의 이야기가 숨 쉬고 있죠. :gray[*(미소)*] 마치 멈춰 서서...
   - [올바른 예시]:
     낡은 담벼락, 빛바랜 간판,
     그리고 사람들의 이야기가 숨 쉬고 있죠.
     :gray[*(미소)*]
     마치 잠시 멈춰 서서...
4. 지문: `:gray[*(지문)*]` 형식으로 대본 전체에서 2~3회만 드물게 사용하며, 지문도 대사와 같은 줄에 쓰지 말고 단독으로 줄바꿈하여 작성하라.
5. 이름 표기: 1인극은 **:blue[이름]**, 2인극은 **:blue[이름]**과 **:green[이름]** 사용. 이름과 대사 사이에도 줄바꿈을 적용해 대본의 가독성을 높여라.
