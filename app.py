...
if not current_keys_main:
    ...
    if st.button("✨ 새로운 대본 생성하기", use_container_width=True):
        ...
        prompt = f"""
성우 공채 전문 작가로서 반드시 [{final_main}] 장르에 완벽하게 맞춰 대본을 집필하라.
...
[핵심 지침 (매우 중요 - 절대 엄수)]:
1. 🚨 카테고리(장르) 고유의 스타일 완벽 적용 및 캐릭터 성격 분리:
   - 대본의 텍스트(글) 자체는 무조건 **[{final_main}] 장르의 정통 스타일**로만 작성하라. 이전 장르의 느낌이 절대 섞이지 않게 하라!
   - 장르가 '애니메이션/게임'이라면, 해당 세계관에 맞는 역동적이고 극적인 대사를 작성하라.
   - 장르가 '내레이션/광고'라면, 철저하게 정돈된 문어체와 정보 전달 중심의 대본을 작성하라.
   - 위 [강제 부여 캐릭터 설정]의 성격과 나이는 성우가 낭독할 때 연기할 '목소리 톤'일 뿐이다. 캐릭터가 '껄렁하다'고 해서 대본 자체의 어휘가 불량해지거나, '다정하다'고 해서 대본이 유치해져서는 안 된다. 오직 선택된 장르의 글만 써라.

2. 분량 (절대 엄수): 
   - 전체 대사는 **최소 8문장 이상**으로 풍성하고 길게 작성하라. 

3. 🚨 줄바꿈 및 대본 형태 (절대 엄수): 
   - 산문(문단) 형태의 작성을 절대 금지한다.
   - 성우의 호흡을 위해 **모든 문장(마침표, 물음표, 느낌표)이 끝날 때마다 반드시 강제로 줄바꿈(엔터)**을 하라.
   - 한 줄에 한 문장 혹은 반 문장만 작성하라.

4. 캐릭터 정보 출력 규칙:
   - 제공된 [강제 부여 캐릭터 설정]을 그대로 양식에 맞춰 출력하라.
   - 🚨 주의: '(여기서부터는 캐릭터 설정을 완전히 무시하고...)' 같은 안내 문구나 설명용 텍스트는 절대 출력하지 마라. 대본 텍스트만 깔끔하게 출력하라.

5. 지문: `:gray[*(지문)*]` 형식으로 대본 전체에서 2~3회만 단독 줄로 분리하라.
6. 이름 표기: 1인극은 **:blue[이름]**, 2인극은 **:blue[이름]**과 **:green[이름]** 사용. 이름 다음에도 무조건 줄바꿈.
7. HTML 태그 사용 금지. {genre_guide}

양식:
### 📋 캐릭터 정보
이름 ({target_age_group}, {user_gender}, [외형 창작], {target_personality})
