...
if not current_keys_main:
    ...
    if st.button("✨ 새로운 대본 생성하기", use_container_width=True):
        ...
        # 🚨 프롬프트: 아예 산문(줄글) 형태를 금지하고 시적인 포맷을 강제합니다.
        prompt = f"""
...
3. 🚨 줄바꿈 및 대본 형태 (가장 중요, 절대 엄수) 🚨: 
   - **산문(줄글) 형태의 작성을 절대 금지한다.** 하나의 문단으로 뭉쳐서 출력하면 실패한 대본이다.
...
[대본 출력 예시 (이 형태를 반드시 지킬 것)]
:blue[김영감]
이른 새벽, 도시의 숨소리가 막 시작될 무렵입니다.

나는 습관처럼 낡은 창가에 서서 바깥을 응시합니다.

수십 년을 변치 않은 풍경 같지만,
자세히 보면 모든 것이 조금씩 달라지고 있죠.

:gray[*(옅은 미소)*]

시간은 덧없이 흘러 모든 것을 바꾸어 놓지만...
...
"""
        ...
        if result == "NO_KEYS":
            ...
            if result:
                ...
                # 🚨 강력한 2차 안전장치: AI가 고집을 부려도 파이썬에서 강제로 문장을 찢어버립니다.
                result = result.replace(". ", ".\n\n")
                result = result.replace("? ", "?\n\n")
                result = result.replace("! ", "!\n\n")
                result = result.replace(") ", ")\n\n")
