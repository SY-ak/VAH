...
[핵심 지침 (매우 중요 - 절대 엄수)]:
1. 🚨 대본 내용 및 말투와 캐릭터 설정의 완벽한 분리 (가장 중요):
   - 위 [강제 부여 캐릭터 설정]은 성우가 임의의 컨셉으로 '연기 연습'을 하기 위한 미션일 뿐이다.
   - **대본 자체의 내용, 단어 선택, 어투, 말투에는 위 캐릭터의 나이나 성격을 절대(Never) 반영하지 마라!**
   - 캐릭터가 '껄렁한 10대'이든 '우울한 노인'이든 상관없이, 대본은 무조건 [{final_main}] 장르의 **가장 정통적이고 중립적인 표준 말투**로만 작성하라. (예시: 불량배 캐릭터가 주어져도, 대본 자체는 아나운서나 교양 다큐멘터리 성우처럼 아주 점잖고 전문적인 표준어로 쓰여 있어야 함)
...
4. 캐릭터 정보 강제 출력 규칙:
   - AI가 장르에 맞춰 성격을 멋대로 순화하거나 변경하는 것을 금지한다. 제공된 [강제 부여 캐릭터 설정]을 그대로 양식에 맞춰 출력하라.
...
### 📖 연습 대본
(여기서부터는 캐릭터 설정을 완전히 무시하고, 오직 장르의 표준적인 대본만 작성할 것)
"""
        result = None
