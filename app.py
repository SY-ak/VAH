...
GENRE_GUIDES = {
    ...
    "내레이션": "정확한 정보 전달 (단, 캐릭터 성격은 강제 부여된 설정을 우선시할 것).",
...
# 🚨 새롭게 추가된 랜덤 성격 리스트
PERSONALITIES = [
    "열혈, 에너지 넘치는", "냉소적, 시니컬한", "껄렁껄렁, 불량배 같은",
    "다정다감, 따뜻한", "소심, 늘 주눅들어 있는", "거만, 귀족적인",
    "엉뚱, 4차원적인", "극도로 진지, 유머감각 제로인", "우울, 세상 다 산 느낌의",
    "하이텐션, 쉴 새 없이 떠드는", "느릿느릿, 여유로운", "예민, 신경질적인",
    "허세 가득, 잘난 척하는", "무미건조, 감정 없는 AI 같은", "광기 어린, 집착하는",
    "단정, FM 원칙주의자 스타일의", "음흉, 속을 알 수 없는", "호탕, 털털하고 호탕한"
]
...
1. 캐릭터 설정: **이름 (나이, 성별, 외형, 말투)** 형식으로 한 줄 요약하라.
   - 필수 조건: 메인 캐릭터의 성별은 **{user_gender}**, 연령대는 **{target_age_group}**이어야 한다.
   - 🚨 성격 강제 부여 (클리셰 파괴): 장르의 뻔한 클리셰(예: 내레이션은 차분해야 함, 애니는 열혈이어야 함 등)를 철저히 무시하라! 메인 캐릭터의 성격과 말투는 반드시 **[{target_personality}]** 컨셉으로 설정하여 대본을 작성하라.
2. 분량: 전체 대사는 **최소 8문장 이상**으로 풍성하게 작성하라.
3. 🚨 줄바꿈 및 대본 형태 (절대 엄수): 
   - **산문(줄글) 형태의 작성을 절대 금지한다.**
   - 성우의 호흡을 위해 **모든 문장(마침표, 물음표, 느낌표)이 끝날 때마다 반드시 강제로 줄바꿈(엔터)**을 하라.
   - 한 줄에 여러 문장을 연달아 쓰지 말고, 무조건 **한 줄에 한 문장 혹은 반 문장**만 작성하라.
4. 지문: `:gray[*(지문)*]` 형식으로 대본 전체에서 2~3회만 드물게 사용하며, 지문은 대사와 섞지 말고 단독 줄로 분리하라.
5. 이름 표기: 1인극은 **:blue[이름]**, 2인극은 **:blue[이름]**과 **:green[이름]** 사용. 이름 다음에도 무조건 줄바꿈을 하라.
...
"""
        result = None
        with st.spinner(f"AI가 연습용 대본을 생성중입니다... (성격: {target_personality})"):
